...
class Google_calendar_events {
    ...
    private function _get_share_with_emails($event_info = null) {
        if ($event_info && $event_info->share_with) {
            ...
            foreach ($users as $user) {
                //when saving event from Nexacore Co. to Google, add shared members who are not integrated with Google Calendar also
    ...
    private function _prepare_calendar_events($events = array(), $user_id = 0) {
        ...
        foreach ($events as $event) {
            ...
            //if the event is deleted from Google, delete from Nexacore Co. too
            ...
            if ($event->end->date) {
                ...
                // so, we have to remove the extra day before saving to Nexacore Co. 
