...
?>
    <div id="js-nexacore-chat-wrapper" class="nexacore-chat-wrapper hide"></div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var $chatBoxWrapper = '<div id="js-nexacore-chat-wrapper" class="nexacore-chat-wrapper hide"></div>';
            if (isMobile()) {
                if (!hideChatIcon) {
            ...
            setChatIcon = function(type, count) {
                ...
                if (type === "open") {
                    $("#js-nexacore-chat-wrapper").addClass("hide"); //hide chat box
                    $("#js-init-chat-icon").removeClass("has-message");
                } else if (type === "close") {
                    $("#js-nexacore-chat-wrapper").removeClass("hide"); //show chat box
                    $("#js-init-chat-icon").removeClass("has-message");
                } else if (type === "unread") {
            ...
            $('body #js-nexacore-chat-wrapper').on('click', '.js-message-row', function() {
                getActiveChat($(this).attr("data-id"));
            });

            $('body #js-nexacore-chat-wrapper').on('click', '.js-message-row-of-team-members-tab', function() {
                getChatlistOfUser($(this).attr("data-id"), "team_members");
            });

            $('body #js-nexacore-chat-wrapper').on('click', '.js-message-row-of-clients-tab', function() {
            ...
            async function restoreChatDimensions() {
                ...
                if (dimensions) {
                    var $chatWrapper = $('#js-nexacore-chat-wrapper');
            ...
            //make resizable
            makeResizable('#js-nexacore-chat-wrapper', {
                minWidth: 310,
                maxWidth: 800,
                minHeight: 400,
            ...
            $(document).on('click', '.reset-chat-dimension', function() {
                ...
                $('#js-nexacore-chat-wrapper').removeAttr('style');
                $('#js-nexacore-chat-wrapper').removeClass('full-screen');
        ...
        function enterChatFullScreen() {
            var $wrapper = $("#js-nexacore-chat-wrapper");
        ...
        function adjustChatBodyHeight() {
            var chatBoxHeight = $('#js-nexacore-chat-wrapper').height();
            var headerHeight = $('.nexacore-chat-header').outerHeight() || 60;
            var footerHeight = $('.nexacore-chat-footer').outerHeight() || 77;

            $('.nexacore-chat-body').height(chatBoxHeight - (headerHeight + footerHeight));
        }
        ...
        function getChatlistOfUser(user_id, tab_type) {
            ...
            appLoader.show({
                container: "#js-nexacore-chat-wrapper",
            ...
            appAjaxRequest({
                ...
                success: function(response) {
                    $("#js-nexacore-chat-wrapper").html(response);
                    appLoader.hide();
                }
            });
        ...
        function loadChatTabs(trigger_from_user_chat) {
            ...
            setTimeout(function() {
                ...
                resizableHandles("#js-nexacore-chat-wrapper");
            }, 300);
        }
